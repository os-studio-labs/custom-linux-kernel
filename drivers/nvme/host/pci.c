...
#ifdef CONFIG_FW_HEALTH
	struct fw_health *fw_health;
#endif
};
...
#ifdef CONFIG_FW_HEALTH
	dev->fw_health = fw_health_alloc(&pdev->dev, 10000, NULL, NULL);
	if (!dev->fw_health)
		dev_warn(&pdev->dev, "failed to allocate FW health monitor\n");
#endif
