		  ...
		  const struct pci_device_id *id)
{
	...
	rtlpriv->fw_health = fw_health_alloc(&pdev->dev, 5000, NULL, NULL);
	if (!rtlpriv->fw_health)
		pr_err("Failed to allocate firmware health monitor\n");
#endif
	/*proximity init here*/
...
fail3:
#ifdef CONFIG_FW_HEALTH
	fw_health_free(rtlpriv->fw_health);
#endif
