...
static int rtl_btc_alloc_variable(struct rtl_priv *rtlpriv, bool wifi_only)
{
	if (wifi_only) {
		rtlpriv->btcoexist.wifi_only_context =
			kzalloc(sizeof(struct wifi_only_cfg), GFP_KERNEL);
		if (!rtlpriv->btcoexist.wifi_only_context)
			return -ENOMEM;
	} else {
...
{
	if (rtl_btc_alloc_variable(rtlpriv, false)) {
		pr_err("Failed to allocate BT coexistence context\n");
		return;
	}
...
{
	if (rtl_btc_alloc_variable(rtlpriv, true)) {
		pr_err("Failed to allocate WiFi-only BT context\n");
		return;
	}
