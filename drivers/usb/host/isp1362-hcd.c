			     ...
			     struct isp1362_ep *ep, u16 len)
{
	...
	WARN_ON(epq->buf_avail > epq->buf_count);
	ep->ptd_index = found;
...
static inline void release_ptd_buffers(struct isp1362_ep_queue *epq, struct isp1362_ep *ep)
{
	...
	WARN_ON(epq->buf_avail > epq->buf_count);
	WARN_ON(epq->ptd_count > epq->buf_count);
...
static void postproc_ep(struct isp1362_hcd *isp1362_hcd, struct isp1362_ep *ep)
{
	...
	*/
	if (cc == PTD_DATAUNDERRUN) {
		if (short_ok) {
			...
			if (usb_pipecontrol(urb->pipe)) {
				...
				WARN_ON(urb->actual_length > urb->transfer_buffer_length);
	...
	case USB_PID_OUT:
		...
		WARN_ON(PTD_GET_COUNT(ptd) != ep->length);
...
static irqreturn_t isp1362_irq(struct usb_hcd *hcd)
{
	...
	if (WARN_ON(isp1362_hcd->irq_active++))
		return IRQ_NONE;
...
static int isp1362_mem_config(struct usb_hcd *hcd)
{
	...
	if (WARN_ON((unsigned)ISP1362_INTL_BUFFERS > 32))
		return -EINVAL;
	if (atl_buffers > 32)
		atl_buffers = 32;
