...
static void assert_priolists(struct i915_sched_engine * const sched_engine)
{
	...
	WARN_ON(rb_first_cached(&sched_engine->queue) !=
			    ...
			    const struct i915_sched_attr *attr)
{
	...
	WARN_ON(prio == I915_PRIORITY_INVALID);
	...
	list_for_each_entry_safe_reverse(dep, p, &dfs, dfs_link) {
		...
		WARN_ON(node_to_request(node)->engine->sched_engine !=
			   sched_engine);
...
void i915_sched_node_reinit(struct i915_sched_node *node)
{
	...
	WARN_ON(!list_empty(&node->signalers_list));
	WARN_ON(!list_empty(&node->waiters_list));
	WARN_ON(!list_empty(&node->link));
}
...
void i915_sched_node_fini(struct i915_sched_node *node)
{
	...
	list_for_each_entry_safe(dep, tmp, &node->signalers_list, signal_link) {
		WARN_ON(!list_empty(&dep->dfs_link));
