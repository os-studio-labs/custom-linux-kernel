...
{
	WARN_ON(type >= INTEL_UC_FW_NUM_TYPES);
...
{
	WARN_ON(uc_fw->status == INTEL_UC_FIRMWARE_UNINITIALIZED);
			    ...
			    bool needs_ggtt_mapping)
{
	...
	WARN_ON(uc_fw->file_selected.path);
					   ...
					   struct intel_uc_fw_file *huc_selected)
{
	...
	if (WARN_ON(!huc_selected->path || !guc_selected->path))
		return -EINVAL;
...
int intel_uc_fw_fetch(struct intel_uc_fw *uc_fw)
{
	...
	if (WARN_ON(!gt->wopcm.size))
		return -EINVAL;
	if (WARN_ON(!intel_uc_fw_is_enabled(uc_fw)))
		return -EINVAL;
...
static u32 uc_fw_ggtt_offset(struct intel_uc_fw *uc_fw)
{
	...
	WARN_ON(gt->type == GT_MEDIA && gt->info.id > 1);
	if (gt->type == GT_MEDIA)
		offset += SZ_8M;

	if (WARN_ON(!drm_mm_node_allocated(node)))
		return 0;
	WARN_ON(upper_32_bits(node->start));
	WARN_ON(upper_32_bits(node->start + node->size - 1));
	WARN_ON(offset + uc_fw->obj->base.size > node->size);
	WARN_ON(uc_fw->obj->base.size > INTEL_UC_RSVD_GGTT_PER_FW);
...
static int uc_fw_xfer(struct intel_uc_fw *uc_fw, u32 dst_offset, u32 dma_flags)
{
	...
	WARN_ON(upper_32_bits(offset) & 0xFFFF0000);
...
int intel_uc_fw_mark_load_failed(struct intel_uc_fw *uc_fw, int err)
{
	...
	if (WARN_ON(!intel_uc_fw_is_loadable(uc_fw)))
		return -EINVAL;
...
int intel_uc_fw_upload(struct intel_uc_fw *uc_fw, u32 dst_offset, u32 dma_flags)
{
	...
	if (WARN_ON(intel_uc_fw_is_loaded(uc_fw)))
		return -EINVAL;
...
static int uc_fw_rsa_data_create(struct intel_uc_fw *uc_fw)
{
	 ...
	 */
	WARN_ON(uc_fw->rsa_size > PAGE_SIZE);
...
size_t intel_uc_fw_copy_rsa(struct intel_uc_fw *uc_fw, void *dst, u32 max_len)
{
	...
	WARN_ON(!intel_uc_fw_is_available(uc_fw));
