...
__active_retire(struct i915_active *ref)
{
	...
	if (WARN_ON(i915_active_is_idle(ref)))
		return;
	...
	WARN_ON(rcu_access_pointer(ref->excl.fence));
	...
	if (ref->cache) {
		...
		WARN_ON(ref->tree.rb_node != &ref->cache->node);
	...
	rbtree_postorder_for_each_entry_safe(it, n, &root, node) {
		WARN_ON(i915_active_fence_isset(&it->base));
...
static struct active_node *__active_lookup(struct i915_active *ref, u64 idx)
{
	...
	WARN_ON(idx == 0); /* 0 is the unordered timeline, rsvd for cache */
...

{
	...
	WARN_ON(node->timeline != engine->kernel_context->timeline->fence_context);
					    ...
					    struct intel_engine_cs *engine)
{
	...
	for_each_engine_masked(engine, gt, mask, tmp) {
		...
		WARN_ON(rcu_access_pointer(node->base.fence) != ERR_PTR(-EAGAIN));

		WARN_ON(barrier_to_engine(node) != engine);
...
void i915_active_acquire_barrier(struct i915_active *ref)
{
	...
	llist_for_each_safe(pos, next, take_preallocated_barriers(ref)) {
		...
		WARN_ON(!intel_engine_pm_is_awake(engine));
...
void i915_request_add_active_barriers(struct i915_request *rq)
{
	...
	if (WARN_ON(!intel_context_is_barrier(rq->context)))
		return;
	if (WARN_ON(intel_engine_is_virtual(engine)))
		return;
	if (WARN_ON(i915_request_timeline(rq) != engine->kernel_context->timeline))
		return;
