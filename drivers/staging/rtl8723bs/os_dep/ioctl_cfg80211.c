	...
	)
{
	...
	spt_band = rtw_zmalloc(sizeof(struct ieee80211_supported_band, GFP_KERNEL) +
				...
				const u8 *mac_addr, struct key_params *params)
{
	...
	param = rtw_malloc(param_len, GFP_KERNEL);
	if (!param)
		return -1;
...
static int rtw_cfg80211_set_probe_req_wpsp2pie(struct adapter *padapter, char *buf, int len)
{
	...
	if (len > 0) {
		...
		if (wps_ie) {
			...
			pmlmepriv->wps_probe_req_ie = rtw_malloc(wps_ielen, GFP_KERNEL);
			if (!pmlmepriv->wps_probe_req_ie)
				return -EINVAL;
...
static int rtw_cfg80211_set_wpa_ie(struct adapter *padapter, u8 *pie, size_t ielen)
{
	...
	buf = rtw_zmalloc(ielen, GFP_KERNEL);
	if (!buf) {
		ret =  -ENOMEM;
		goto exit;
	}
				...
				struct cfg80211_connect_params *sme)
{
	...
	if ((psecuritypriv->dot11AuthAlgrthm == dot11AuthAlgrthm_Shared ||
		...
		if (wep_key_len > 0) {
			...
			pwep = rtw_malloc(wep_total_len, GFP_KERNEL);
			if (!pwep) {
				ret = -ENOMEM;
				goto exit;
			}
...
static int rtw_cfg80211_add_monitor_if(struct adapter *padapter, char *name, struct net_device **ndev)
{
	...
	/*  wdev */
	mon_wdev = rtw_zmalloc(sizeof(struct wireless_dev, GFP_KERNEL));
	if (!mon_wdev) {
		ret = -ENOMEM;
		goto out;
	}
...
static int rtw_add_beacon(struct adapter *adapter, const u8 *head, size_t head_len, const u8 *tail, size_t tail_len)
{
	...
	pbuf = rtw_zmalloc(head_len + tail_len, GFP_KERNEL);
	if (!pbuf)
		return -ENOMEM;
...
int rtw_wdev_alloc(struct adapter *padapter, struct device *dev)
{
	...
	*((struct adapter **)wiphy_priv(wiphy)) = padapter;
	...
	/*  wdev */
	wdev = rtw_zmalloc(sizeof(struct wireless_dev, GFP_KERNEL));
	if (!wdev) {
		ret = -ENOMEM;
