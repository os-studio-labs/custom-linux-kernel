...
bool out_of_memory(struct oom_control *oc)
{
	...
	/*
	 * SentraOS OOM killer — uses the Rust evaluator instead of
	 * the legacy select_bad_process heuristic.
	 */
	if (IS_ENABLED(CONFIG_SENTRAOS_OOM_RUST)) {
		struct sentraos_oom_victim victim;
		struct task_struct *p;

		victim = sentraos_oom_evaluate(oc->totalpages,
					       oc->constraint,
					       oc->chosen ? task_pid_nr(oc->chosen) : 0);
		if (victim.pid != 0) {
			/* Find the task by PID */
			rcu_read_lock();
			p = find_task_by_vpid(victim.pid);
			if (p) {
				get_task_struct(p);
				oc->chosen = p;
			}
			...
			if (oc->chosen && oc->chosen != (void *)-1UL) {
				sentraos_oom_log_kill(&victim);
				oom_kill_process(oc,
					!is_memcg_oom(oc) ?
					"Out of memory (SentraOS ML scorer)" :
