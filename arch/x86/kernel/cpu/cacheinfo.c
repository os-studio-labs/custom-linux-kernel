	...
	if ((num_cache_leaves == 0 || c->x86 == 15) && c->cpuid_level > 1) {
		...
		for (i = 0 ; i < n ; i++) {
			...
			for (j = 1 ; j < 16 ; j++) {
				...
				struct rust_cache_desc rdesc;
				if (rust_fill_cache_descriptor(des, &rdesc)) {
					if (only_trace && rdesc.cache_type != RUST_CACHE_TRACE)
						break;
					switch (rdesc.cache_type) {
					case RUST_CACHE_L1_INST:
						l1i += rdesc.size_kb;
						break;
					case RUST_CACHE_L1_DATA:
						l1d += rdesc.size_kb;
						break;
					case RUST_CACHE_L2:
						l2 += rdesc.size_kb;
