...
#include <linux/energy_model_rust.h>
	       ...
	       struct cpumask *pd_cpus, struct task_struct *p, int dst_cpu)
{
	...
	/*
	 * If a Rust EnergyModel is registered, use it instead of the C EM.
	 * The Rust EM is scheduler-class-agnostic and supports pluggable
	 * model implementations (ACPI, DT-based, learned, etc.).
	 */
	if (rust_em_is_available()) {
		u32 cpu = dst_cpu >= 0 ? dst_cpu : cpumask_first(pd_cpus);
		energy = (unsigned long)rust_em_estimate(cpu,
			 (u32)max_util, (u32)busy_time);
	} else {
		energy = em_cpu_energy(pd->em_pd, max_util,
				       busy_time, eenv->cpu_cap);
	}
