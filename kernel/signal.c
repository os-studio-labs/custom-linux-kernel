...
#include <linux/sentraos_signal.h>
...
{
	if (IS_ENABLED(CONFIG_SENTRAOS_SIGNAL_RUST)) {
		signal_clear_all(task_pid_nr(t));
		   ...
		   kernel_siginfo_t *info, enum pid_type *type)
{
	if (IS_ENABLED(CONFIG_SENTRAOS_SIGNAL_RUST)) {
		...
		*type = PIDTYPE_PID;
		signo = signal_dequeue(task_pid_nr(tsk), &sinfo);

		if (signo) {
			clear_siginfo(info);
			info->si_signo = sinfo.signo;
			info->si_errno = sinfo.errno;
			info->si_code = sinfo.code;
			info->si_pid = sinfo.pid;
			info->si_uid = sinfo.uid;
		}
			...
			enum pid_type type)
{
	...
	if (IS_ENABLED(CONFIG_SENTRAOS_SIGNAL_RUST)) {
		struct signal_info sinfo = {
			.signo = sig,
			.errno = info ? info->si_errno : 0,
			.code = info ? info->si_code : SI_USER,
			.pid = info ? info->si_pid : 0,
			.uid = info ? info->si_uid : 0,
			.value = 0,
		};

		if (type == PIDTYPE_PID) {
			ret = signal_send(task_pid_nr(p), sig, &sinfo, 0);
		} else {
			ret = signal_send_group(task_tgid_nr(p), sig, &sinfo);
		}
		return ret;
	}
	...
	enum sig_handler handler)
{
	...
	if (IS_ENABLED(CONFIG_SENTRAOS_SIGNAL_RUST)) {
		...
		/* Force: override block/ignore */
		signal_unblock(task_pid_nr(t), 1ULL << sig);
		return signal_send(task_pid_nr(t), sig, &sinfo, 1);
	}
